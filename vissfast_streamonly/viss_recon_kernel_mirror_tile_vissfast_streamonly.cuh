...
struct GroupArtifactsHost {
  ...
  std::vector<int> offsets;        // blockage=1 generic
  std::vector<int> idx_sorted;     // blockage=1 generic
  std::vector<int> rep_half_idx;   // blockage=1 occ_mode=2 representative
};
    ...
    GroupArtifactsHost& hout)
{
  ...
  if(blockage==0){
    ...
    if(occ_mode==2){
      hout.rep_half_idx.assign(nuniq, 0);
      for(int q=0; q<nuniq; ++q){
        int s0 = hout.offsets[q];
        int s1 = hout.offsets[q+1];
        int L = s1 - s0;
        hout.rep_half_idx[q] = (L>0) ? hout.idx_sorted[s0 + (L>>1)] : 0;
      }
      hout.offsets.clear(); hout.offsets.shrink_to_fit();
      hout.idx_sorted.clear(); hout.idx_sorted.shrink_to_fit();
    }
...
__global__ void gather_rep_half_inputs(
    const int* __restrict__ rep_half_idx,
    int nuniq,
    ...
    float2* __restrict__ repV,
    float4* __restrict__ repP1,
    float4* __restrict__ repP2)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if(i >= nuniq) return;
  int bih = rep_half_idx[i];
  int bi = halfidx_to_fullpos_dev(bih);
  repV[i] = Viss_half[bih];
  repP1[i] = make_float4(x1_full[bi], y1_full[bi], z1_full[bi], invn1_full[bi]);
  repP2[i] = make_float4(x2_full[bi], y2_full[bi], z2_full[bi], invn2_full[bi]);
}
    ...
    uint32_t* __restrict__ Wseg)
{
  ...
  __shared__ float shU[CHUNK_KEYS];
  __shared__ float shVg[CHUNK_KEYS];
  __shared__ float2 shVV[CHUNK_KEYS];
  __shared__ float4 shP1[CHUNK_KEYS];
  __shared__ float4 shP2[CHUNK_KEYS];
...
struct GpuCtx {
  ...
  int* d_keys_buf=nullptr;
  int* d_repidx_buf=nullptr;
  float* d_ugu_buf=nullptr;
  float* d_vgu_buf=nullptr;
  float2* d_repV_buf=nullptr;
  float4* d_repP1_buf=nullptr;
  float4* d_repP2_buf=nullptr;
