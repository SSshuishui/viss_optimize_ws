...
int main(int argc,char** argv){
  ...
  for(int gi=0; gi<G; ++gi){
    ...
    ctx[gi].ntile = (int)((n_chunk + VISS_TILE_PIX_HOST - 1) / VISS_TILE_PIX_HOST);
  ...
  for(int day_idx=0; day_idx<day_count; ++day_idx){
    ...
    for(int s=0;s<segs;s++){
      ...
      for(int gi=0; gi<G; ++gi){
        ...
        viss_partial_all_halfsym_tilecone<TILE_PIX,true><<<gridV,BLOCKV,shmem,stream>>>(
      ...
      for(int i=0;i<segN_half;i++){
        double re=0, im=0;
        for(int gi=0; gi<G; ++gi){
          re += (double)ctx[gi].slots[cur].h_Vpart[i].x;
          im += (double)ctx[gi].slots[cur].h_Vpart[i].y;
        }
