...
struct HostTimer { using clock=std::chrono::high_resolution_clock; clock::time_point t0; void tic(){t0=clock::now();} double toc_s() const {return std::chrono::duration_cast<std::chrono::duration<double>>(clock::now()-t0).count();} };

static inline std::string get_arg(int argc,char** argv,const std::string& key,const std::string& defv){
  for(int i=1;i<argc;i++){ std::string s(argv[i]); if(s.rfind(key+"=",0)==0) return s.substr(key.size()+1); }
  return defv;
}
...
static inline std::string norm_dir(std::string p){ while(!p.empty() && p.back()=='/') p.pop_back(); return p; }
...
static void write_txt_viss2(const std::string& fn, const float2* v, size_t n){
  std::ofstream ofs(fn);
  if(!ofs.is_open()){ std::cerr<<"ERROR open "<<fn<<"\n"; std::exit(1); }
...
// ----- grouping (device0) -----
__global__ void iota_kernel(int* idx, int n){ int i=blockIdx.x*blockDim.x+threadIdx.x; if(i<n) idx[i]=i; }
__global__ void build_locg_kernel(const float* __restrict__ u, const float* __restrict__ v, int* __restrict__ locg, int n, float inv_du, int RES, int half){
  int i=blockIdx.x*blockDim.x+threadIdx.x;
  if(i<n){
...
__global__ void set_last_offset(int* offsets, int nuniq, int n){ if(blockIdx.x==0 && threadIdx.x==0) offsets[nuniq]=n; }
__global__ void gather_viss_by_idx(const float2* __restrict__ viss_in, const int* __restrict__ idx_sorted, float2* __restrict__ viss_sorted, int n){
  int i=blockIdx.x*blockDim.x+threadIdx.x;
...
struct Float2AddOp { __host__ __device__ float2 operator()(const float2& a,const float2& b) const { return make_float2(a.x+b.x,a.y+b.y); } };
    ...
    float* __restrict__ Cseg)
{
  ...
  for(int base=0;base<nuniq;base+=CHUNK_KEYS){
    ...
    for(int k=0;k<CHUNK_KEYS;k++){
      int key=shK[k]; if(key==0) continue;
      int tmp=key-1;
      int U=tmp/RES;
      int V=tmp-U*RES;
      int ui=U-half;
  ...
  for(int base=0;base<nuniq;base+=CHUNK_KEYS){
    ...
    if(t<CHUNK_KEYS){
      ...
      if(q<nuniq){
        ...
        shO0[t]=offsets[q];
        shO1[t]=offsets[q+1];
      } else { shK[t]=0; shO0[t]=0; shO1[t]=0; }
    }
    __syncthreads();

    #pragma unroll
    for(int kk=0; kk<CHUNK_KEYS; kk++){
      int key=shK[kk]; if(key==0) continue;
      int s0=shO0[kk], s1=shO1[kk];
      int L=s1-s0; if(L<=0) continue;
      ...
      auto handle_one = [&](int pos){
        ...
        if(c1>=cosphi && c2>=cosphi){
          float2 vv=Viss[bi];
          if(isfinite(vv.x) && isfinite(vv.y)){ sumV.x+=vv.x; sumV.y+=vv.y; cnt++; }
        }
      };
...
__global__ void add_inplace(float* dst,const float* src,long long n){ long long i=(long long)blockIdx.x*blockDim.x+threadIdx.x; if(i<n) dst[i]+=src[i]; }
__global__ void add_inplace_u32(uint32_t* dst,const uint32_t* src,long long n){ long long i=(long long)blockIdx.x*blockDim.x+threadIdx.x; if(i<n) dst[i]+=src[i]; }
__global__ void scale_inplace(float* dst,long long n,float inv){ long long i=(long long)blockIdx.x*blockDim.x+threadIdx.x; if(i<n) dst[i]*=inv; }
__global__ void normalize_by_weight(float* C,const uint32_t* W,long long n){ long long i=(long long)blockIdx.x*blockDim.x+threadIdx.x; if(i<n){ uint32_t w=W[i]; C[i]= (w>0)? (C[i]/(float)w) : 0.0f; } }
...
static void compute_fa_fb_host_matlab_no_intercept(const float* u,const float* v,const float* w,int n,float &fa,float &fb,double &denom_out){
  ...
  if(!std::isfinite((double)a) || !std::isfinite((double)b)){ a=0; b=0; }
...
struct GpuCtx {
  ...
  float2* d_Vpart=nullptr;   // N
  float2* d_Viss=nullptr;    // N
...
int main(int argc,char** argv){
  ...
  if(btag!="1M" && btag!="10M" && btag!="30M"){ std::cerr<<"ERROR --btag\n"; return 1; }
  if(nside<=0){ if(btag=="1M") nside=512; else if(btag=="10M") nside=4096; else nside=16384; }
  if(segs<=0){ if(btag=="1M") segs=1; else if(btag=="10M") segs=10; else segs=30; }
  ...
  for(int d: gpus){ if(d<0 || d>=devCount){ std::cerr<<"ERROR gpu id "<<d<<" out of range "<<devCount<<"\n"; return 1; } }
  int G=(int)gpus.size();
  ...
  std::cout<<"in_dir="<<in_dir<<" sky_dir="<<sky_dir<<" out_dir="<<out_dir<<" gpus="<<gpus_s<<" uvw_max="<<uvw_max<<" B_clip="<<B_clip<<"\n";
  ...
  if(!ok) ok=load_single_auto_np(fB2,hB.data(),npix) && load_single_auto_np(fT2,hTheta.data(),npix) && load_single_auto_np(fP2,hPhi.data(),npix);
  ...
  constexpr int BASELINES_PER_T=56;
  if(N % BASELINES_PER_T != 0){
    std::cerr<<"ERROR: N="<<N<<" not divisible by 56; cannot time-segment safely.\n";
    return 1;
  }

  int T = N / BASELINES_PER_T;
  ...
  for(int gi=0; gi<G; ++gi){
    ...
    ctx[gi].dev=dev; ctx[gi].pix0=pix0; ctx[gi].pix1=pix1; ctx[gi].n_chunk=n_chunk; ctx[gi].N=N;
    ...
    // Viss buffers (no batch)
    CHECK_CUDA(cudaMalloc(&ctx[gi].d_Vpart,(size_t)N*sizeof(float2)));
    CHECK_CUDA(cudaMalloc(&ctx[gi].d_Viss,(size_t)N*sizeof(float2)));
    CHECK_CUDA(cudaMallocHost(&ctx[gi].h_Vpart,(size_t)N*sizeof(float2)));
  ...
  // ---------------- Viss compute (one shot, no batch) ----------------
  HostTimer t_v; t_v.tic();
  constexpr int TILE_PIX=256;
  const int BLOCKV=256;
  int gridV=(N + BLOCKV - 1)/BLOCKV;
  ...
  for(int gi=0; gi<G; ++gi){
    ...
    viss_partial_all<TILE_PIX,true><<<gridV,BLOCKV,shmem,stream>>>(
    ...
    CHECK_CUDA(cudaPeekAtLastError());
    CHECK_CUDA(cudaMemcpyAsync(ctx[gi].h_Vpart, ctx[gi].d_Vpart, (size_t)N*sizeof(float2), cudaMemcpyDeviceToHost, stream));
    CHECK_CUDA(cudaStreamSynchronize(stream));
  }

  // host reduce across GPUs -> hViss
  std::vector<float2> hViss(N);
  for(int i=0;i<N;i++){
    double re=0, im=0;
    for(int gi=0; gi<G; ++gi){
  ...
  std::cout<<"Computed Viss (host-reduced) in "<<t_v.toc_s()<<" s\n";

  // phase-correct on host, then upload corrected Viss to each GPU
  HostTimer t_vup; t_vup.tic();
  phase_correct_viss_host(hViss.data(), hw.data(), N);

  #pragma omp parallel for num_threads(G)
  for(int gi=0; gi<G; ++gi){
    CHECK_CUDA(cudaSetDevice(ctx[gi].dev));
    cudaStream_t stream=ctx[gi].stream;
    CHECK_CUDA(cudaMemcpyAsync(ctx[gi].d_Viss, hViss.data(), (size_t)N*sizeof(float2), cudaMemcpyHostToDevice, stream));
    CHECK_CUDA(cudaStreamSynchronize(stream));
  }
  std::cout<<"Phase-corrected and uploaded Viss in "<<t_vup.toc_s()<<" s\n";

  if(write_viss){
    std::string fv=out_dir+"Viss"+std::to_string(day)+"day"+btag+".txt";
    write_txt_viss2(fv, hViss.data(), (size_t)N);
  ...
  for(int s=0;s<segs;s++){
    int t0=seg_t0[s];
    int tlen=seg_tlen[s];
    int row0=t0*BASELINES_PER_T;
    int segN=tlen*BASELINES_PER_T;
    if(segN<=0) continue;
    ...
    for(int gi=0; gi<G; ++gi){
      ...
      int b=256; int g2=(int)((ctx[gi].n_chunk+b-1)/b);
    ...
    std::cout<<"[seg "<<(s+1)<<"/"<<segs<<"] row0="<<row0<<" segN="<<segN
             <<" fa="<<fa<<" fb="<<fb<<" denom="<<denom<<" nuniq="<<grp.nuniq<<" RES="<<RES;
    if(print_stats){
  ...
  if(!ofs.is_open()){ std::cerr<<"ERROR open "<<outC<<"\n"; return 1; }
