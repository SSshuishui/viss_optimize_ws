...
int main(int argc,char** argv){
  ...
  if(N % SIGNED_BASELINES_PER_T != 0){
    std::cerr<<"ERROR: N="<<N<<" not divisible by "<<SIGNED_BASELINES_PER_T<<"; cannot time-segment safely.\n";
    return 1;
  }
  ...
  int T = N / SIGNED_BASELINES_PER_T;
  int N_half = T * UNIQUE_BASELINES_PER_T;
  ...
  for(int gi=0; gi<G; ++gi){
    ...
    ctx[gi].dev=dev; ctx[gi].pix0=pix0; ctx[gi].pix1=pix1; ctx[gi].n_chunk=n_chunk; ctx[gi].N=N; ctx[gi].N_half=N_half;
  ...
  // ---------------- Viss compute (one shot, half-sym explicit baselines) ----------------
  ...
  for(int gi=0; gi<G; ++gi){
    ...
    CHECK_CUDA(cudaPeekAtLastError());
    CHECK_CUDA(cudaMemcpyAsync(ctx[gi].h_Vpart, ctx[gi].d_Vpart, (size_t)N_half*sizeof(float2), cudaMemcpyDeviceToHost, stream));
    CHECK_CUDA(cudaStreamSynchronize(stream));
  }

  // host reduce across GPUs -> hViss_half
  ...
  std::cout<<"Computed half-sym Viss (host-reduced) in "<<t_v.toc_s()<<" s, explicit baselines="<<N_half<<" / "<<N<<"\n";

  // phase-correct compact half, then expand to full and upload
  HostTimer t_vup; t_vup.tic();
  phase_correct_viss_halfsym_host(hViss_half.data(), hw.data(), N_half);

  std::vector<float2> hViss(N, make_float2(0,0));
  expand_viss_halfsym_to_full(hViss_half, hViss);
  ...
  std::cout<<"Phase-corrected, conjugate-expanded and uploaded Viss in "<<t_vup.toc_s()<<" s\n";
